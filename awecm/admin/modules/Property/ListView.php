...
$xtpl = new XTemplate("modules/Property/ListView.html");
...
if(!empty($_REQUEST['name'])) {
  ...
  $where[] = "title LIKE '%".$db->escape($name)."%' ";
}
...
if(!empty($_REQUEST['location'])) {
  ...
  $where[] = "location = '".$db->escape($location)."'";
} else $location = '';
...
$navigation   = _build_navigation($row_count, $offset, 6);
...
if($response['list']) {
  ...
  foreach($response['list'] as $seed) {
    ...
    $row['is_featured'] = 'No';
    if(strpos($row['tag'],'featured') !== false){
        $row['is_featured'] = 'Yes';
    }
    ...
    $row['location'] = $location_dom[$row['location']];
...
require_once("modules/Property/Search.php");
