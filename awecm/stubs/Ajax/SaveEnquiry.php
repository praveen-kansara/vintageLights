...
if(isset($_REQUEST['type']) && $_REQUEST['type'] == "ajax") {
  ...
  // removed the validation due to errors - 10-Jul-2023    
  $valid_email = 1; //check_email_validity($_REQUEST['email']);
  ...
  if($valid_email) {
      ...
      if(!empty($admin_email)) {
          ...
          $cust_message  = (!empty($message)) ? $message : '';
        
          $admin_email_subject = 'Inquiry from '.$cust_name.' on '.$email_site_name.' website';
          
          $admin_email_body = "<p style='color: #333; font-size: 15px;margin-top:0px;margin-bottom: 15px;'>".$cust_name." (".$email.") $cust_phone sent the following message;</p><br/>$cust_message";
