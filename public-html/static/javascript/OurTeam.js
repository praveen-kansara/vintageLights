...
OURTEAM.OURTEAM = function() {
  this.initialize();
}

OURTEAM.OURTEAM.prototype = {
  initialize: function() {
    this.setupBasics();
    this.slugSetUp();
    this.setUpSideBar();
  ...
  setUpSideBar: function() {
    ...
    $("#btn_save_publish").click(function() {
      ...
      $('#publish_date').attr('required','required');
      if(tinyMCE.activeEditor.hidden) {
        tinyMCE.execCommand('mceToggleEditor', false, 'content');
      }
      $('#form').submit();
  ...
  slugSetUp: function () {
    ...
    $("#link_set_featured_image").click(function(e) {
      e.preventDefault();
      self.socialImageSetUp();
      });
    $("#remove_featured_image").click(function(e) {
      e.preventDefault();
      $(".preview-social-share-image").find('.social-share-img').attr('src', '');
      $("input[name='featured_image']").val("");
      $('#remove_featured_image').addClass('hidden');
    });
  },
  socialImageSetUp: function() {
    var self = this;
    var modalTemplate = $("#image_lib_widget");
    modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
    modalTemplate.find("#btn_insert_image").click(function() {
      var selectedImage = $(".images-list .selected .med-image");
      var imageSource = selectedImage.attr('data-original-path');
      $(".preview-social-share-image").find('.social-share-img').attr('src', imageSource);
      $("input[name='featured_image']").val(selectedImage.attr('data-media-id'));
      $('#remove_featured_image').removeClass('hidden');
      modalTemplate.hide();
    });
...
$(document).ready( function() {
  ...
  $('#vcard_select_error').hide();
  var vcard_pdf_path = $('#vcard_path').val();
  if (vcard_pdf_path == '') {
    $('#input_vcard').show();
    $('#preview_vcard').hide();
  } else {
  ...
  $("#vcard").change(function () {
    var validExtensions = ["vcf"];
    var file = $(this).val().split('.').pop();
    if (validExtensions.indexOf(file) == -1) {
      $('#vcard_select_error').show();
      $(this).val('');
    } else $('#vcard_select_error').hide();
  });
