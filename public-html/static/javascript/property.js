...
PROPERTY.PORTFOLIO = function() {
  this.initialize();
}

PROPERTY.PORTFOLIO.prototype = {
  initialize: function() {
    this.setupBasics();
    this.slugSetUp();
    this.setUpSideBar();
  ...
  secondBannerImageSetUp: function() {
    var self = this;
    var modalTemplate = $("#image_lib_widget");
    modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
    modalTemplate.find("#btn_insert_image").click(function() {
      var selectedImage = $(".images-list .selected .med-image");
      var imageSource = selectedImage.attr('data-original-path');
      $("#second-banner-image").find('.second-banner-img').attr('src', imageSource);
      $("input[name='second_banner_image']").val(selectedImage.attr('data-media-id'));
      modalTemplate.hide();
    });
    modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
      modalTemplate.hide();
  ...
  floorPlanImageSetUp: function() {
    ...
    modalTemplate.find("#btn_insert_image").click(function() {
      var selectedImage = $(".images-list .selected .med-image");
      var imageSource = selectedImage.attr('data-original-path');
      $("#floor-plan-image").find('.floor-plan-img').attr('src', imageSource);
      $("input[name='floor_plan_image']").val(selectedImage.attr('data-media-id'));
      modalTemplate.hide();
    });
    modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
      modalTemplate.hide();
  ...
  floorPlanZoomImageSetUp: function() {
    var self = this;
    var modalTemplate = $("#image_lib_widget");
    modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
    modalTemplate.find("#btn_insert_image").click(function() {
      var selectedImage = $(".images-list .selected .med-image");
      var imageSource = selectedImage.attr('data-original-path');
      $("#floor-plan-zoom-image").find('.floor-plan-zoom-img').attr('src', imageSource);
      $("input[name='floor_plan_zoom_image']").val(selectedImage.attr('data-media-id'));
      modalTemplate.hide();
    });
    modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
      modalTemplate.hide();
    ...
    floorPlan2ImageSetUp: function() {
        ...
        modalTemplate.find("#btn_insert_image").click(function() {
          var selectedImage = $(".images-list .selected .med-image");
          var imageSource = selectedImage.attr('data-original-path');
          $("#floor-plan2-image").find('.floor-plan2-img').attr('src', imageSource);
          $("input[name='floor_plan2_image']").val(selectedImage.attr('data-media-id'));
          modalTemplate.hide();
        });
        modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
          modalTemplate.hide();
    ...
    floorPlan2ZoomImageSetUp: function() {
        var self = this;
        var modalTemplate = $("#image_lib_widget");
        modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
        modalTemplate.find("#btn_insert_image").click(function() {
          var selectedImage = $(".images-list .selected .med-image");
          var imageSource = selectedImage.attr('data-original-path');
          $("#floor-plan2-zoom-image").find('.floor-plan2-zoom-img').attr('src', imageSource);
          $("input[name='floor_plan2_zoom_image']").val(selectedImage.attr('data-media-id'));
          modalTemplate.hide();
        });
        modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
          modalTemplate.hide();
    ...
    floorPlan3ImageSetUp: function() {
        ...
        modalTemplate.find("#btn_insert_image").click(function() {
          var selectedImage = $(".images-list .selected .med-image");
          var imageSource = selectedImage.attr('data-original-path');
          $("#floor-plan3-image").find('.floor-plan3-img').attr('src', imageSource);
          $("input[name='floor_plan3_image']").val(selectedImage.attr('data-media-id'));
          modalTemplate.hide();
        });
        modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
          modalTemplate.hide();
    ...
    floorPlan3ZoomImageSetUp: function() {
        var self = this;
        var modalTemplate = $("#image_lib_widget");
        modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
        modalTemplate.find("#btn_insert_image").click(function() {
          var selectedImage = $(".images-list .selected .med-image");
          var imageSource = selectedImage.attr('data-original-path');
          $("#floor-plan3-zoom-image").find('.floor-plan3-zoom-img').attr('src', imageSource);
          $("input[name='floor_plan3_zoom_image']").val(selectedImage.attr('data-media-id'));
          modalTemplate.hide();
        });
        modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
          modalTemplate.hide();
  ...
  videoPlaceholderImageSetUp: function() {
    var self = this;
    var modalTemplate = $("#image_lib_widget");
    modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
    modalTemplate.find("#btn_insert_image").click(function() {
      var selectedImage = $(".images-list .selected .med-image");
      var imageSource = selectedImage.attr('data-original-path');
      $("#video-placeholder-image").find('.video-placeholder-img').attr('src', imageSource);
      $("input[name='video_placeholder_image']").val(selectedImage.attr('data-media-id'));
      modalTemplate.hide();
    });
    modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
      modalTemplate.hide();
  ...
  brochureSetUp: function() {
    var self = this;
    var modalTemplate = $("#brochure_lib_widget");
    // modalTemplate.find('.footer').html('<button id="btn_insert_image" type="button" class="_custom-button">Insert Image</button><button id="btn_close_popup" type="button" class="_custom-button">Cancel</button>');
    modalTemplate.find("#btn_close_popup,.close-popup").click(function() {
      modalTemplate.hide();
    });
  ...
  setUpSideBar: function() {
    ...
    $("#btn_save_draft").click(function() {
      $("#status").val("draft");
      var propertyImages = [];
      var propertyImageContainer = $("#property_images");
      var selectedImages = propertyImageContainer.find('.appended .med-image');
      selectedImages.each(function(i, data) {
        propertyImages.push($(this).attr('data-media-id'));
      });
      $("input[name='property_images']").val(propertyImages.join());
    ...
    $("#link_upload_brochure").click(function(e) {
      e.preventDefault();
      self.brochureSetUp();
    });
    $("#upload_property_brochure").click(function(e) {
      e.preventDefault();
      self.uploadPropertyBrochure();
    });
  ...
  setupBasics: function() {
    ...
    $("#second-banner-image").click(function() {
      self.secondBannerImageSetUp();
    });
    
    $("#floor-plan-image").click(function() {
      self.floorPlanImageSetUp();
    });
    
    $("#floor-plan-zoom-image").click(function() {
      self.floorPlanZoomImageSetUp();
    });

    $("#floor-plan2-image").click(function() {
        self.floorPlan2ImageSetUp();
        });
        
    $("#floor-plan2-zoom-image").click(function() {
    self.floorPlan2ZoomImageSetUp();
    });
    
    
    $("#floor-plan3-image").click(function() {
    self.floorPlan3ImageSetUp();
    });
    
    $("#floor-plan3-zoom-image").click(function() {
    self.floorPlan3ZoomImageSetUp();
    });
        
        
    $("#video-placeholder-image").click(function() {
      self.videoPlaceholderImageSetUp();
    });
    $("#insert_content_image").click(function() {
      self.contentImageSetUp();
    });
    $(".attach-property-images").click(function() {
      self.propertyImageSetUp();
    });
    $(".image-lib-grid").on('click', '.icon-delete-property-image', function() {
    ...
    $("#remove-second-banner").click(function() {
      $('.second-banner-img').attr('src', '');
      $("input[name='second_banner_image']").val('');
    });
    $("#floor-plan-banner").click(function() {
      $('.floor-plan-img').attr('src', '');
      $("input[name='floor_plan_image']").val('');
    });
    
    $("#floor-plan-zoom-banner").click(function() {
      $('.floor-plan-zoom-img').attr('src', '');
      $("input[name='floor_plan_zoom_image']").val('');
    });

    $("#remove-video-placeholder-image").click(function() {
      $('.video-placeholder-img').attr('src', '');
      $("input[name='video_placeholder_image']").val('');
    });
...
$(function() {
  ...
  $('#brocure_select_error').hide();
  var brochure_pdf_path = $('#pdf_path').val();
  if (brochure_pdf_path == '') {
    $('#input_brochure').show();
    $('#preview_brochure').hide();
  } else {
    $('#preview_brochure').show();
    $('#input_brochure').hide();
  }

  $('#remove_brochure').click(function() {
    $('#pdf_path').val('');
    $('#input_brochure').show();
    $('#preview_brochure').hide();
  });
  ...
  $("#brochure").change(function () {
    ...
    if (validExtensions.indexOf(file) == -1) {
      $('#brocure_select_error').show();
      $(this).val('');
    } else $('#brocure_select_error').hide();
  });
